...
module T = Tls_types.Bindings(struct let funptr fn = Foreign.funptr fn end)
include T.Openssl_basic

type ssl_method = unit -> unit ptr
let ssl_method : ssl_method fn = void @-> returning (ptr void)

type ssl_ctx = unit ptr
let ssl_ctx = ptr void

type ssl = unit ptr
let ssl = ptr void

type ssl_session = unit ptr
let ssl_session = ptr void
